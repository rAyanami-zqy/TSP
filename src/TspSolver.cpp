#define TSP_SOLVER_IMPLEMENTATION_CONTEXT 1
...
void BranchBoundSolver::setNodeAscentTraceOutput(
    std::ostream& output, std::size_t max_depth)
{
    node_ascent_trace_output_ = &output;
    node_ascent_trace_max_depth_ = max_depth;
}

void BranchBoundSolver::disableNodeAscentTraceOutput()
{
    node_ascent_trace_output_ = nullptr;
    node_ascent_trace_max_depth_ = 0;
}
...
void BranchBoundSolver::setNodeAscentSiblingWarmStartStrategy(
    SiblingWarmStartStrategy strategy)
{
    sibling_warm_start_strategy_ = strategy;
    if (strategy == SiblingWarmStartStrategy::Off) {
        sibling_warm_potential_.clear();
    }
}

void BranchBoundSolver::setInitialClkStrategy(
    InitialClkStrategy strategy, double gap_ratio,
    std::size_t additional_starts)
{
    if (!isFinite(gap_ratio) || gap_ratio < 0.0) {
        throw std::invalid_argument(
            "adaptive CLK gap ratio must be finite and non-negative");
    }
    initial_clk_strategy_ = strategy;
    adaptive_clk_gap_ratio_ = gap_ratio;
...
void BranchBoundSolver::setBranchLiftFirstDepth(std::size_t depth)
{
    branch_lift_first_depth_ = depth;
}

void BranchBoundSolver::setBranchSplitZeroGainDepth(std::size_t depth)
{
    branch_split_zero_gain_depth_ = depth;
}
...
void BranchBoundSolver::setPotentialUpdateShallowDepthTier(
    std::size_t depth, std::size_t iterations)
{
    if ((depth == 0) != (iterations == 0)) {
        throw std::invalid_argument(
            "potential update shallow depth and iterations must both be zero or positive");
    }
    potential_update_shallow_depth_ = depth;
    potential_update_shallow_iterations_ = iterations;
}

void BranchBoundSolver::setPotentialUpdateSlowWarmExtend(
    std::size_t depth, std::size_t iterations)
{
    if ((depth == 0) != (iterations == 0)) {
        throw std::invalid_argument(
            "potential update slow-warm depth and iterations must both be zero or positive");
    }
    potential_update_slow_warm_depth_ = depth;
    potential_update_slow_warm_extend_iterations_ = iterations;
}
...
std::size_t BranchBoundSolver::basePotentialUpdateIterationLimit(
    int depth, double bound, double upper_bound) const
{
    if (usesLargeGapPotentialUpdateTier(bound, upper_bound)) {
        return potential_update_large_gap_iterations_;
    }
    if (depth > 0 && potential_update_shallow_iterations_ != 0
        && static_cast<std::size_t>(depth)
            <= potential_update_shallow_depth_) {
        return potential_update_shallow_iterations_;
    }
    return potential_update_iterations_;
}
    ...
    int depth, double bound, double upper_bound) const
{
    const std::size_t base =
        basePotentialUpdateIterationLimit(depth, bound, upper_bound);
    if (depth > 0 && potential_update_slow_warm_extend_iterations_ != 0
        && static_cast<std::size_t>(depth)
            <= potential_update_slow_warm_depth_
        && potential_update_slow_warm_extend_iterations_ > base) {
        return potential_update_slow_warm_extend_iterations_;
    }
    return base;
}
    ...
    std::size_t forced_edge_count) const
{
    ...
    if (potentialUpdateIterationLimit(depth, bound, upper_bound) == 0) {
    ...
    int depth) const
{
    ...
    result.milestone_bounds.fill(current_bound);
    constexpr std::array<std::size_t, 7> milestone_rounds{
        1, 2, 4, 8, 16, 32, 64};
    auto record_milestone = [&](std::size_t round) {
        for (std::size_t index = 0; index < milestone_rounds.size(); ++index) {
            if (round != milestone_rounds[index]) continue;
            result.milestone_bounds[index] = result.bound;
            result.milestone_reached[index] = 1;
            break;
        }
    };
    const std::size_t base_iterations = depth > 0
        ? basePotentialUpdateIterationLimit(depth, current_bound, upper_bound)
        : max_iterations;
    const std::size_t observe_rounds =
        base_iterations > 16 ? 16 : 0;
    bool had_gain_by_observe = false;
    auto has_strict_gain = [&] {
        const double gain_tolerance = scaledRoundoffTolerance(
            result.bound, current_bound, static_cast<std::size_t>(n_));
        return result.bound > current_bound + gain_tolerance;
    };
    // 浅层慢热：基础上限之后只有“16 轮仍无改善、此刻已经抬升”才继续。
    auto stop_after_ascent_round = [&](std::size_t round) {
        if (round == observe_rounds && observe_rounds != 0) {
            had_gain_by_observe = has_strict_gain();
        }
        if (max_iterations <= base_iterations || round != base_iterations) {
            return false;
        }
        if (depth <= 0
            || potential_update_slow_warm_extend_iterations_ == 0
            || static_cast<std::size_t>(depth)
                > potential_update_slow_warm_depth_) {
            return true;
        }
        if (observe_rounds != 0 && had_gain_by_observe) {
            return true;
        }
        if (!has_strict_gain()) {
            return true;
        }
        result.slow_warm_extended = true;
        return false;
    };
    ...
    Evaluation guarded_initial_evaluation;
    bool reuse_guarded_initial_evaluation = false;
    if (sibling_warm_start_strategy_ != SiblingWarmStartStrategy::Off
        && node_ascent_sibling_warm_weight_ > 0.0
        && sibling_warm_potential_.size() == static_cast<std::size_t>(n_)) {
        const std::vector<double> parent_potentials = potentials;
        ...
        if (!finite_warm_start) {
            potentials = parent_potentials;
        } else if (sibling_warm_start_strategy_
                   == SiblingWarmStartStrategy::Guarded) {
            // 父势在 current_bound 中已有当前约束图上的有效证书。只额外
            // 评估一次混合兄弟势，并在严格改善该证书时采用；否则回退父势。
            // 这次验证也属于实际 1-tree 工作量，因此计入 iterations。
            result.sibling_warm_probed = true;
            const Evaluation warm_evaluation = evaluate(potentials);
            ++result.iterations;
            const double tolerance = warm_evaluation.feasible
                ? scaledRoundoffTolerance(
                    warm_evaluation.bound, current_bound,
                    static_cast<std::size_t>(n_))
                : 0.0;
            if (warm_evaluation.feasible
                && warm_evaluation.bound > current_bound + tolerance) {
                result.sibling_warm_accepted = true;
                result.bound = warm_evaluation.bound;
                result.potentials = potentials;
                guarded_initial_evaluation = warm_evaluation;
                reuse_guarded_initial_evaluation = true;
            } else {
                result.sibling_warm_rejected = true;
                potentials = parent_potentials;
            }
        }
    }
    auto remember_sibling_warm_start = [&]() {
        ...
        if (sibling_warm_start_strategy_ == SiblingWarmStartStrategy::Off
            || node_ascent_sibling_warm_weight_ <= 0.0 || !result.feasible
            || !std::all_of(
                potentials.begin(), potentials.end(),
                [](double value) { return isFinite(value); })) {
            return;
    ...
    if (node_ascent_strategy_ == NodeAscentStrategy::Helsgaun) {
        ...
        for (std::size_t iteration = 0;
             ...
             ++iteration) {
            ++result.ascent_rounds;
            Evaluation evaluation;
            if (reuse_guarded_initial_evaluation) {
                evaluation = std::move(guarded_initial_evaluation);
                reuse_guarded_initial_evaluation = false;
            } else {
                evaluation = evaluate(potentials);
                ++result.iterations;
            }
            ...
            record_milestone(result.ascent_rounds);
            ...
            if (stop_after_ascent_round(result.ascent_rounds)) break;
    ...
    int depth, bool count_node)
{
    ...
    const int parent_epoch_depth = current_potential_epoch_depth_;
    ++result_.stats.search_node_potential_updates_triggered;
    ++potential_updates_in_round_;
    const double upper_bound_at_update = best_cost_;
    const bool uses_large_gap_tier = usesLargeGapPotentialUpdateTier(
        current_tree.cost, upper_bound_at_update);
    if (uses_large_gap_tier) {
        ++result_.stats.potential_updates_large_gap_tier;
    }
    const bool uses_shallow_depth_tier =
        !uses_large_gap_tier
        && potential_update_shallow_iterations_ != 0
        && static_cast<std::size_t>(depth)
            <= potential_update_shallow_depth_;
    if (uses_shallow_depth_tier) {
        ++result_.stats.potential_updates_shallow_depth_tier;
    }
    const std::size_t iteration_limit = potentialUpdateIterationLimit(
        depth, current_tree.cost, upper_bound_at_update);
    const NodePotentialUpdateResult update = updateNodePotentialBound(
        node, current_tree.cost, upper_bound_at_update,
        iteration_limit, depth);
    ...
    if (update.slow_warm_extended) {
        ++result_.stats.potential_updates_slow_warm_extended;
    }
    ...
    if (update.sibling_warm_probed) {
        ++result_.stats.sibling_warm_probes;
    }
    if (update.sibling_warm_accepted) {
        ++result_.stats.sibling_warm_accepted;
    }
    if (update.sibling_warm_rejected) {
        ++result_.stats.sibling_warm_rejected;
    }
    result_.stats.potential_update_seconds += std::chrono::duration<double>(
        std::chrono::steady_clock::now() - update_started_at).count();

    const bool trace_update = node_ascent_trace_output_ != nullptr
        && (node_ascent_trace_max_depth_ == 0
            || static_cast<std::size_t>(depth)
                <= node_ascent_trace_max_depth_);
    auto write_trace = [&](const char* outcome,
                           std::size_t epoch_subtree_created,
                           std::size_t epoch_subtree_expanded,
                           std::size_t epoch_subtree_pruned,
                           std::size_t epoch_descendant_updates,
                           std::size_t epoch_descendant_rebuilds,
                           double upper_bound_after) {
        if (!trace_update) return;
        const double scale = std::max(
            1.0, std::fabs(upper_bound_at_update));
        const double initial_gap = std::max(
            0.0, upper_bound_at_update - current_tree.cost);
        const double final_gap = update.feasible && isFinite(update.bound)
            ? std::max(0.0, upper_bound_at_update - update.bound)
            : initial_gap;
        const double gap_closed_ratio = initial_gap > 0.0
            ? std::max(0.0, initial_gap - final_gap) / initial_gap
            : 0.0;
        const double tolerance = update.feasible && isFinite(update.bound)
            ? scaledRoundoffTolerance(
                update.bound, current_tree.cost,
                static_cast<std::size_t>(n_))
            : 0.0;
        const bool improved = update.feasible
            && update.bound > current_tree.cost + tolerance;
        const bool prunable = improved
            && shouldPrune(update.bound, upper_bound_at_update);
        *node_ascent_trace_output_
            << result_.stats.search_node_potential_updates_triggered << ','
            << depth << ','
            << parent_epoch_depth << ','
            << depth - parent_epoch_depth << ','
            << node.forced_edges.size() << ','
            << iteration_limit << ','
            << update.iterations << ','
            << update.ascent_rounds << ','
            << std::setprecision(17) << current_tree.cost << ','
            << update.bound << ','
            << upper_bound_at_update << ','
            << initial_gap / scale << ','
            << final_gap / scale << ','
            << gap_closed_ratio << ','
            << (improved ? 1 : 0) << ','
            << (prunable ? 1 : 0) << ','
            << (update.stopped_prunable ? 1 : 0) << ','
            << (update.probe_started ? 1 : 0) << ','
            << (update.probe_continued ? 1 : 0) << ','
            << (update.probe_rejected ? 1 : 0) << ','
            << (uses_large_gap_tier ? 1 : 0) << ','
            << (uses_shallow_depth_tier ? 1 : 0) << ','
            << (update.slow_warm_extended ? 1 : 0);
        for (std::size_t index = 0;
             index < update.milestone_bounds.size(); ++index) {
            *node_ascent_trace_output_ << ',';
            if (update.milestone_reached[index] == 0 || initial_gap <= 0.0) {
                continue;
            }
            const double milestone_gain = std::max(
                0.0, update.milestone_bounds[index] - current_tree.cost);
            *node_ascent_trace_output_ << milestone_gain / initial_gap;
        }
        *node_ascent_trace_output_
            << ',' << outcome
            << ',' << epoch_subtree_created
            << ',' << epoch_subtree_expanded
            << ',' << epoch_subtree_pruned
            << ',' << epoch_descendant_updates
            << ',' << epoch_descendant_rebuilds
            << ',' << upper_bound_after
            << ',' << std::max(
                0.0, upper_bound_at_update - upper_bound_after)
            << '\n';
    };
    ...
    if (update.probe_rejected || !update.feasible
        ...
        || update.bound <= current_tree.cost + tolerance) {
        write_trace("rejected", 0, 0, 0, 0, 0, upper_bound_at_update);
        return false;
    }
    ...
    if (shouldPrune(update.bound, best_cost_)) {
        ...
        write_trace("pruned", 0, 0, 1, 0, 0, upper_bound_at_update);
        return true;
    }
    ...
    if (!epoch_tree.feasible) {
        restore_epoch();
        write_trace("rebuild-failed", 0, 0, 0, 0, 0,
                    upper_bound_at_update);
        return false;
    }
    ...
    const std::size_t subtree_created_before = result_.stats.nodes_created;
    const std::size_t subtree_expanded_before = result_.stats.nodes_expanded;
    const std::size_t subtree_pruned_before =
        result_.stats.nodes_pruned_by_bound;
    const std::size_t descendant_updates_before =
        result_.stats.search_node_potential_updates_triggered;
    const std::size_t descendant_rebuilds_before =
        result_.stats.potential_updates_rebuilt;
    ...
    write_trace(
        "installed",
        result_.stats.nodes_created - subtree_created_before,
        result_.stats.nodes_expanded - subtree_expanded_before,
        result_.stats.nodes_pruned_by_bound - subtree_pruned_before,
        result_.stats.search_node_potential_updates_triggered
            - descendant_updates_before,
        result_.stats.potential_updates_rebuilt
            - descendant_rebuilds_before,
        best_cost_);
    restore_epoch();
    return true;
}
...
SolveResult BranchBoundSolver::solve()
{
    ...
    writeDebugLine(
        ...
        "initial tour timing: seconds="
            ...
            + " clk_starts="
            + std::to_string(result_.stats.initial_clk_starts));
    ...
    while (true) {
        ...
        // Adaptive 先用单起点 CLK 得到 UB 和首个根证书；gap 足够大才追加
        // 多起点 CLK。若 UB 改善，必须重新优化根势，因为 Polyak 步长依赖 UB。
        if (initial_clk_strategy_ == InitialClkStrategy::Adaptive
            && !adaptive_initial_clk_attempted_
            && adaptive_clk_additional_starts_ != 0
            && !initial_tour_alternatives_.empty()
            && isFinite(best_cost_) && isFinite(root_tree.cost)
            && !shouldPrune(root_tree.cost, best_cost_)) {
            const double gap = std::max(0.0, best_cost_ - root_tree.cost);
            const double relative_gap = gap
                / std::max(1.0, std::fabs(best_cost_));
            if (relative_gap >= adaptive_clk_gap_ratio_) {
                adaptive_initial_clk_attempted_ = true;
                ++result_.stats.adaptive_clk_triggers;
                const double old_cost = best_cost_;
                const std::size_t old_clk_starts =
                    result_.stats.initial_clk_starts;
                const auto adaptive_clk_started =
                    std::chrono::steady_clock::now();
                const bool improved = improveInitialTourDiversified(
                    initial_tour_alternatives_, root_tree.cost,
                    best_tour_, best_cost_, adaptive_clk_additional_starts_);
                result_.stats.initial_tour_seconds +=
                    std::chrono::duration<double>(
                        std::chrono::steady_clock::now()
                            - adaptive_clk_started).count();
                const std::size_t starts_run =
                    result_.stats.initial_clk_starts - old_clk_starts;
                initial_tour_alternatives_.erase(
                    initial_tour_alternatives_.begin(),
                    initial_tour_alternatives_.begin()
                        + static_cast<std::ptrdiff_t>(std::min(
                            starts_run,
                            initial_tour_alternatives_.size())));
                if (improved) {
                    const double recomputed_cost = tourCost(best_tour_);
                    if (recomputed_cost < old_cost) {
                        best_cost_ = recomputed_cost;
                        result_.stats.initial_upper_bound = best_cost_;
                        ++result_.stats.adaptive_clk_improvements;
                        rememberCandidateHintTour(best_tour_);
                        writeDebugLine(
                            debug_,
                            "adaptive CLK improved: cost="
                                + formatDebugDouble(best_cost_)
                                + " previous="
                                + formatDebugDouble(old_cost)
                                + " relative_gap="
                                + formatDebugDouble(relative_gap)
                                + " starts="
                                + std::to_string(starts_run));
                        continue;
                    }
                }
                writeDebugLine(
                    debug_,
                    "adaptive CLK retained incumbent: cost="
                        + formatDebugDouble(best_cost_)
                        + " relative_gap="
                        + formatDebugDouble(relative_gap)
    ...
    result_.stats.replacement_seconds = replacement_seconds_;
    {
        ...
        line << "exact solve finished: feasible=" << (result_.feasible ? "yes" : "no")
             ...
             << result_.stats.potential_updates_large_gap_tier
             << " potential_shallow_depth_tier="
             << result_.stats.potential_updates_shallow_depth_tier
             << " potential_slow_warm_extended="
             << result_.stats.potential_updates_slow_warm_extended
             << " branch_lift_first_reorders="
             << result_.stats.branch_lift_first_reorders
             << " branch_zero_gain_splits="
             << result_.stats.branch_zero_gain_splits
             << " branch_ascent_strong_probes="
             << result_.stats.branch_ascent_strong_probes
             << " branch_ascent_strong_seconds="
             << formatDebugDouble(
                    result_.stats.branch_ascent_strong_seconds)
...
double BranchBoundSolver::trialBranchAscentBound(
    const PartialSol& node, const OneTree& current_tree,
    const Edge& tree_edge, bool force_edge,
    std::size_t ascent_iterations) const
{
    if (ascent_iterations == 0) {
        if (force_edge) {
            return currentForceBranchBound(node, current_tree, tree_edge);
        }
        const double delta =
            currentForbidReplacementDelta(
                const_cast<PartialSol&>(node), current_tree, tree_edge);
        return isFinite(delta)
            ? current_tree.cost + delta
            : std::numeric_limits<double>::infinity();
    }

    const std::size_t edge_id = edgeId(tree_edge.u, tree_edge.v);
    ...
    PartialSol trial = node;
    if (force_edge) {
        if (trial.forced_degree[static_cast<std::size_t>(tree_edge.u)] >= 2
            || trial.forced_degree[static_cast<std::size_t>(tree_edge.v)] >= 2) {
            return std::numeric_limits<double>::infinity();
        }

        auto forced_find = [&](int vertex) {
            // 与正式可回滚 DSU 一样不做路径压缩。
            while (trial.forced_parent[static_cast<std::size_t>(vertex)]
                   != vertex) {
                vertex = trial.forced_parent[static_cast<std::size_t>(vertex)];
            }
            return vertex;
        };
        int root_u = forced_find(tree_edge.u);
        int root_v = forced_find(tree_edge.v);
        if (root_u == root_v
            && trial.forced_comp_size[static_cast<std::size_t>(root_u)] < n_) {
            return std::numeric_limits<double>::infinity();
        }

        trial.forced[edge_id] = 1;
        ++trial.forced_degree[static_cast<std::size_t>(tree_edge.u)];
        ++trial.forced_degree[static_cast<std::size_t>(tree_edge.v)];
        trial.forced_edges.push_back(tree_edge);
        if (tree_edge.u != 0 && tree_edge.v != 0) {
        ...
        trial.forbidden[edge_id] = 1;
        if (!trial.candidate_mask.empty()) {
            trial.candidate_mask[edge_id] = 0;
        }
    }
    ...
    for (const Edge& candidate : candidates_sorted_) {
        ...
        if (trial.forbidden[id]) continue;
        if (isCandidateActive(node, id)) {
            trial_candidates.push_back(candidate);
        }
    }
    // 复用正式 force 分支的过滤语义：新 force 导致的度满、
    // forced 子回路和已决定边都不得留在试算候选图中。
    if (!buildBranchCandidates(trial, trial_candidates)) {
        return std::numeric_limits<double>::infinity();
    }
    ...
    const OneTree trial_tree = computeOneTree(trial, trial_candidates);
    if (!trial_tree.feasible || !isFinite(trial_tree.cost)) {
        return std::numeric_limits<double>::infinity();
    }

    const NodePotentialUpdateResult update = updateNodePotentialBound(
        trial, trial_tree.cost, best_cost_, ascent_iterations, 1);
    if (!update.feasible || !isFinite(update.bound)) {
        return std::numeric_limits<double>::infinity();
    }
    return update.bound;
}
...
BranchBoundSolver::BranchSet BranchBoundSolver::bpPartition(
    PartialSol& node, OneTree& work_tree, int depth)
{
    ...
    while (true) {
        ...
        auto consider_branch_edge = [&](const Edge& candidate) {
            if (branch_edge_order_
                ...
                || branch_edge_order_
                    == BranchEdgeOrder::AscentStrongBranchingTop2) {
        ...
        if (branch_edge_order_
            ...
            double best_product_score =
                -std::numeric_limits<double>::infinity();
            const bool use_ascent_score = branch_edge_order_
                == BranchEdgeOrder::AscentStrongBranchingTop2
                && potential_update_strategy_ != PotentialUpdateStrategy::None
                && potential_update_iterations_ != 0
                && depth >= 0 && depth <= 2;
            const std::size_t ascent_score_iterations = use_ascent_score
                ? std::min<std::size_t>(16, potential_update_iterations_)
                : 0;
            const auto ascent_score_started_at =
                std::chrono::steady_clock::now();
            for (const Edge* candidate : strong_candidates) {
                if (candidate == nullptr) continue;
                double force_gain = 0.0;
                double forbid_gain = 0.0;
                if (use_ascent_score) {
                    result_.stats.branch_ascent_strong_probes += 2;
                    const double force_bound = trialBranchAscentBound(
                        node, work_tree, *candidate, true,
                        ascent_score_iterations);
                    const double forbid_bound = trialBranchAscentBound(
                        node, work_tree, *candidate, false,
                        ascent_score_iterations);
                    force_gain = isFinite(force_bound)
                        ? std::max(0.0, force_bound - work_tree.cost)
                        : std::numeric_limits<double>::infinity();
                    forbid_gain = isFinite(forbid_bound)
                        ? std::max(0.0, forbid_bound - work_tree.cost)
                        : std::numeric_limits<double>::infinity();
                } else {
                    forbid_gain = currentForbidReplacementDelta(
    ...
    bool allow_potential_anchor)
{
    ...
    BranchSet B_set = bpPartition(node, current_tree, depth);
    ...
    auto search_force_child = [&](std::size_t idx) {
        const BranchChoice& choice = B_set[idx];
        const Edge& branch_edge = choice.edge;
        const double parent_bound = current_tree.cost;
        ForceChanges force_changes;
        if (apply_force(branch_edge, force_changes)) {
            ...
            if (ok) {
                ...
                if (tree_valid) {
                    Edge extra_edge;
                    bool split = false;
                    const bool try_split =
                        branch_split_zero_gain_depth_ != 0
                        && depth >= 0
                        && static_cast<std::size_t>(depth)
                            <= branch_split_zero_gain_depth_
                        && costsNumericallyEqual(
                            parent_bound, current_tree.cost,
                            static_cast<std::size_t>(n_));
                    if (try_split) {
                        const std::size_t forced_id =
                            edgeId(branch_edge.u, branch_edge.v);
                        double best_forbid_delta =
                            -std::numeric_limits<double>::infinity();
                        for (const Edge& edge : current_tree.edges) {
                            const std::size_t id = edgeId(edge.u, edge.v);
                            if (id == forced_id) continue;
                            ...
                            const bool share_u = edge.u == branch_edge.u
                                || edge.v == branch_edge.u;
                            const bool share_v = edge.u == branch_edge.v
                                || edge.v == branch_edge.v;
                            if (!share_u && !share_v) continue;
                            const int hotspot =
                                share_u ? branch_edge.u : branch_edge.v;
                            if (current_tree.degree[
                                    static_cast<std::size_t>(hotspot)] <= 2) {
                                continue;
                            }
                            const double delta = currentForbidReplacementDelta(
                                node, current_tree, edge);
                            if (!split || delta > best_forbid_delta) {
                                extra_edge = edge;
                                best_forbid_delta = delta;
                                split = true;
                            }
                        }
                    }
                    if (split) {
                        ++result_.stats.branch_zero_gain_splits;
                        const std::size_t split_tree_checkpoint =
                            tree_undo_.size();
                        const std::size_t split_candidate_checkpoint =
                            candidate_undo_.size();
                        const std::size_t extra_id =
                            edgeId(extra_edge.u, extra_edge.v);
                        node.forbidden[extra_id] = 1;
                        deactivateCandidate(node, extra_id);
                        bool forbid_valid = false;
#ifndef TSP_DISABLE_INCREMENTAL_ONETREE
                        forbid_valid = updateOneTreeAfterForbid(
                            node, candidates_sorted_, current_tree,
                            extra_edge, true);
#endif
                        if (!forbid_valid) {
                            rollbackOneTree(
                                current_tree, split_tree_checkpoint);
                            OneTree rebuilt = computeOneTree(
                                node, candidates_sorted_);
                            forbid_valid = rebuilt.feasible;
                            if (forbid_valid) {
                                replaceOneTreeWithRebuild(
                                    current_tree, std::move(rebuilt));
                            }
                        ...
                        ++result_.stats.nodes_created;
                        if (forbid_valid
                            && !shouldPrune(current_tree.cost, best_cost_)) {
                            search(node, current_tree, depth + 1);
                        } else if (!forbid_valid) {
                            ++result_.stats.nodes_pruned_infeasible;
                        } else {
                            ++result_.stats.nodes_pruned_by_bound;
                        }
                        rollbackOneTree(current_tree, split_tree_checkpoint);
                        rollbackCandidates(node, split_candidate_checkpoint);
                        node.forbidden[extra_id] = 0;
                        if (restart_search_requested_) {
                            return;
                        }
                        ForceChanges extra_force;
                        if (apply_force(extra_edge, extra_force)) {
                            const bool extra_ok = filterActiveCandidates(
                                node, extra_force.merged_scan_root,
                                extra_force.merged_scan_count,
                                extra_force.merged_scan_bits);
                            if (extra_ok) {
                                bool extra_tree_valid = true;
#ifndef TSP_DISABLE_INCREMENTAL_ONETREE
                                if (candidate_undo_.size()
                                    != split_candidate_checkpoint) {
                                    extra_tree_valid =
                                        updateOneTreeAfterActiveRemoval(
                                            node, current_tree,
                                            split_candidate_checkpoint, true);
                                }
#else
                                extra_tree_valid = false;
#endif
                                if (!extra_tree_valid) {
                                    rollbackOneTree(
                                        current_tree, split_tree_checkpoint);
                                    OneTree rebuilt = computeOneTree(
                                        node, candidates_sorted_);
                                    extra_tree_valid = rebuilt.feasible;
                                    if (extra_tree_valid) {
                                        replaceOneTreeWithRebuild(
                                            current_tree, std::move(rebuilt));
                                    }
                            ...
                            rollbackOneTree(
                                current_tree, split_tree_checkpoint);
                            rollbackCandidates(
                                node, split_candidate_checkpoint);
                            revert_force(extra_force);
                        } else {
                            ++result_.stats.nodes_pruned_infeasible;
                        }
                    } else {
    ...
    auto apply_forbid_prefix = [&](std::size_t idx) {
        const BranchChoice& choice = B_set[idx];
        const Edge& branch_edge = choice.edge;
    ...
    const bool lift_first =
        branch_lift_first_depth_ != 0
        && depth >= 0
        && static_cast<std::size_t>(depth) <= branch_lift_first_depth_
        && B_set.size() >= 2;
    if (lift_first) {
        ++result_.stats.branch_lift_first_reorders;
        apply_forbid_prefix(0);
        for (std::size_t idx = 1; idx < B_set.size(); ++idx) {
            search_force_child(idx);
            if (restart_search_requested_) break;
            if (idx + 1 == B_set.size()) break;
            apply_forbid_prefix(idx);
        }
        if (!restart_search_requested_) {
            clear_forbid_prefix();
            search_force_child(0);
        }
    } else {
...
#undef TSP_SOLVER_IMPLEMENTATION_CONTEXT
